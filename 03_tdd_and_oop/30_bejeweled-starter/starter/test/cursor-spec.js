...
describe ('Cursor', function () {
  ...
  it('allows you to select a piece', function () {
    cursor.select();
    expect([cursor.selectedRow, cursor.selectedCol]).to.deep.equal([0,0]);
    expect(cursor.selectColor).to.equal("red");
    cursor.down()
    cursor.down()
    cursor.select();
    expect([cursor.selectedRow, cursor.selectedCol]).to.deep.equal([2,0]);
